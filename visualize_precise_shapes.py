...
def generate_transformed_dataset(base_dir="tests/precise_shapes", output_dir="precise_transformation_samples", num_samples=20):
    ...
    for i in range(num_samples):
        ...
        for var in range(3):
            ...
            trans_filename = f"sample_{sample_idx:03d}_transformed_{bg_name}_v{var+1}.jpg"
    ...
    for config in configs[:4]:  # Show first 4 configs
        ...
        cv2.putText(label_img, config["name"].replace("_", " ").title(), (10, 60), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
        cv2.putText(label_img, f"Rotation: ±{config['rotation_range']}°", (10, 100), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 100, 100), 1)
        cv2.putText(label_img, f"Scale: {config['scale_range']}", (10, 130), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 100, 100), 1)
        objects_text = ", ".join([f"{k}: {v}" for k, v in list(config["object_counts"].items())[:2]])
        cv2.putText(label_img, objects_text, (10, 160), 
    ...
    cv2.putText(header, "FastSCP with Precise Segmentation Masks", (400, 40), 
               cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 0), 2)
    
    # Add column labels
    col_labels = np.ones((40, grid.shape[1], 3), dtype=np.uint8) * 250
    cv2.putText(col_labels, "Configuration", (80, 25), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1)
    cv2.putText(col_labels, "White BG", (420, 25), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1)
    cv2.putText(col_labels, "Gradient BG", (720, 25), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1)
    cv2.putText(col_labels, "Pattern BG", (1040, 25), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1)
    ...
    for i in range(2):  # 2 sets of comparisons
        for bg_name in ["gradient", "pattern"]:
            ...
            cv2.putText(label_strip, f"{bg_name} background", (10, 20), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1)
...
def create_html_viewer(metadata_file="precise_transformation_samples/metadata.json", 
    ...
    # Calculate stats
    avg_time = sum(s["transform_time_ms"] for s in metadata["samples"]) / len(metadata["samples"])
...
<body>
    ...
    html_parts.append(f'        <p>Average transformation time: <span class="performance">{avg_time:.2f} ms</span></p>')
    ...
    <div class="sample-group">
        ...
        if group["transforms"]:
            ...
            html_parts.append(f'            <strong>Rotation:</strong> ±{t["rotation_range"]}°<br>')
            html_parts.append(f'            <strong>Scale Range:</strong> {t["scale_range"]}<br>')
            html_parts.append(f'            <strong>Blend Mode:</strong> {t["blend_mode"]} (smooth edges)<br>')
        
        html_parts.append('''        </div>
        <div class="images">
            <div class="image-container">''')
        html_parts.append(f'                <img src="precise_transformation_samples/images/{orig_file}" alt="Original">')
        html_parts.append('''                <div class="image-label">Original</div>
            </div>''')
        ...
        for t in group["transforms"]:
            html_parts.append('''
            <div class="image-container">''')
            html_parts.append(f'                <img src="precise_transformation_samples/images/{t["transformed"]}" alt="Transformed">')
            html_parts.append(f'                <div class="image-label">Variant {t["variation"]} ({t["transform_time_ms"]:.2f} ms)</div>')
            html_parts.append('''            </div>''')
