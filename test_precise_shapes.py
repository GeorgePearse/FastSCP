...
def test_precise_shapes():
    ...
    for config_idx, config in enumerate(configs):
        ...
        for bg_name, bg_img in backgrounds.items():
            ...
            comparison = np.hstack(
                [cv2.resize(bg_img, (400, 300)), cv2.resize(augmented, (400, 300))]
            )
        ...
        cv2.putText(
            header,
            f"Scale: {config['scale']}, Rotation: ±{config['rotation']}°",
            (400, 40),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            (200, 200, 200),
    ...
    for cat_name in ["red_square", "green_circle", "blue_triangle", "purple_star"]:
        ...
        if objects:
            ...
            final = (
                rgb * alpha[:, :, np.newaxis] + checker * (1 - alpha[:, :, np.newaxis])
            ).astype(np.uint8)
            viz[y_off : y_off + new_h, 410 : 410 + new_w] = final

            # Labels
            cv2.putText(
                viz, cat_name, (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2
            )
            cv2.putText(
                viz,
                "Original",
