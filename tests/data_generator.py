...
class TestDataGenerator:
    ...
    def generate_image_with_shapes(
        self, image_id: int, width: int = 640, height: int = 480, num_shapes: int = 5
    ) -> Tuple[np.ndarray, List[COCOAnnotation]]:
        ...
        for _ in range(num_shapes):
            ...
            if shape_type == "rectangle":
                ...
                annotations.append(
                    self.create_shape_annotation("rectangle", [x, y, w, h], image_id)
                )
                ...
                annotations.append(
                    self.create_shape_annotation("circle", bbox, image_id)
                )
                ...
                annotations.append(
                    self.create_shape_annotation("triangle", bbox, image_id)
                )

        return image, annotations

    def generate_dataset(
        self, num_images: int = 5, shapes_per_image: int = 4
    ) -> Dict[str, Any]:
        ...
        print(
            f"Generated dataset with {len(images)} images and {len(annotations)} annotations"
        )
