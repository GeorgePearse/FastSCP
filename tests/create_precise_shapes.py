...
class PreciseShapeGenerator:
    ...
    def create_individual_shapes(self):
        ...
        for i, (name, _, _) in enumerate(shapes, 1):
            self.coco_dataset["categories"].append(
                {"id": i, "name": name, "supercategory": "shape"}
            )
        ...
        for cat_id, (name, shape_type, color_name) in enumerate(shapes, 1):
            ...
            final_img = (shape_img * mask_3ch + white_bg * (1 - mask_3ch)).astype(
                np.uint8
            )
    ...
    def create_composite_scenes(self):
        ...
        for bg_idx, (bg_name, background) in enumerate(backgrounds):
            ...
            for scene_idx in range(2):
                ...
                selected_shapes = np.random.choice(
                    len(shape_configs), num_shapes, replace=False
                )
                ...
                cv2.imwrite(
                    str(self.masks_dir / f"mask_{filename[:-4]}.png"), scene_mask
                )
    ...
    def save_annotations(self):
        ...
        print(
            f"- {len(self.coco_dataset['annotations'])} annotations with precise segmentations"
        )
