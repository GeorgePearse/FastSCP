...
def generate_circle_polygon(
    center_x: int, center_y: int, radius: int, num_points: int = 32
) -> List[float]:
...
def generate_regular_polygon(
    center_x: int, center_y: int, radius: int, num_sides: int, rotation: float = 0
) -> List[float]:
...
def create_segmented_shapes_dataset(output_dir: str = "tests/segmented_shapes"):
        ...
        for shape_type, cx, cy, size, color_name in shapes_config:
            ...
            if shape_type == "square":
                ...
                for i in range(5):
                    ...
                    outer_pts.append(
                        [cx + size * np.cos(angle), cy + size * np.sin(angle)]
                    )
                    # Inner points
                    angle = 2 * np.pi * (i + 0.5) / 5 - np.pi / 2
                    inner_pts.append(
                        [
                            cx + size * 0.4 * np.cos(angle),
                            cy + size * 0.4 * np.sin(angle),
                        ]
                    )
    ...
    for shape_idx, (shape_name, cat_id) in enumerate(
        ...
        if shape_name == "square":
            ...
            pts_list = generate_regular_polygon(cx, cy, size, 5, -np.pi / 2)
            pts = np.array(
                [[pts_list[i], pts_list[i + 1]] for i in range(0, len(pts_list), 2)],
                np.int32,
            )
            cv2.fillPoly(img, [pts], color)
            cv2.fillPoly(mask, [pts], 255)
            ...
            for i in range(5):
                ...
                angle = 2 * np.pi * (i + 0.5) / 5 - np.pi / 2
                inner_pts.append(
                    [cx + size * 0.4 * np.cos(angle), cy + size * 0.4 * np.sin(angle)]
                )
            star_pts = []
            for i in range(5):
                star_pts.append(outer_pts[i])
        ...
        # Add to dataset
        coco_dataset["images"].append(
            {"id": image_id, "width": 300, "height": 300, "file_name": filename}
        )
