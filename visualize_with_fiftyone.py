...
def generate_transformed_dataset(
    base_dir="tests/colored_shapes", output_dir="fiftyone_output", num_samples=20
):
    ...
    base_images = {
        ...
        "gradient": cv2.imread(
            str(Path(base_dir) / "images" / "background_gradient.jpg")
        ),
    ...
    for img_data in coco_data["images"]:
        ...
        for ann in anns:
            ...
            detection = fo.Detection(
                label=label, bounding_box=[rel_x, rel_y, rel_w, rel_h], confidence=1.0
            )
