...
def visualize_segmentation_crops():
    ...
    for cat_name in categories:
        ...
        if objects:
            ...
            vis_img[y_offset : y_offset + new_h, x_offset : x_offset + new_w] = (
                obj_resized
            )
            ...
            masked_obj = (obj_resized * mask_norm + checker * (1 - mask_norm)).astype(
                np.uint8
            )
            vis_img[y_offset : y_offset + new_h, x_offset : x_offset + new_w] = (
                masked_obj
    ...
    if comparison_images:
        ...
        print("   Saved: segmentation_crops_comparison.jpg")
    ...
    cv2.putText(
        labels, "Original", (150, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2
    )
    cv2.putText(
        labels,
        "BBox-based Crop",
    ...
    cv2.imwrite(
        str(output_dir / "bbox_vs_segmentation_comparison.jpg"), final_comparison
    )
