...
class SimpleCopyPasteSegmented(ImageOnlyTransform):
    """
    ...
    def apply(self, image: np.ndarray, **params) -> np.ndarray:
        ...
        for category, count in self.object_counts.items():
            if count > 0:
                objects = self.coco_loader.get_random_objects(category, count, self.include_context)
                objects_to_paste.extend([(obj, mask, category) for obj, mask in objects])
    ...
    def _rotate_image_and_mask(self, image: np.ndarray, mask: np.ndarray, angle: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _paste_object_with_mask(self, image: np.ndarray, obj_img: np.ndarray, 
                               obj_mask: np.ndarray, target_shape: Tuple[int, int]) -> np.ndarray:
        """
        ...
        mask_resized = cv2.resize(obj_mask, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
    ...
    def _blend_with_mask(self, background: np.ndarray, foreground: np.ndarray, 
        ...
        if self.blend_mode == "alpha":
            ...
            blended = (foreground_rgb * mask_smooth + 
                      background * (1 - mask_smooth)).astype(np.uint8)
            
        elif self.blend_mode == "overlay":
            # Simple overlay - use mask as hard cutout
            mask_binary = (mask > 0.5).astype(np.float32)
            blended = (foreground_rgb * mask_binary + 
                      background * (1 - mask_binary)).astype(np.uint8)
            
        elif self.blend_mode == "mix":
            # Mix with random alpha
            alpha = random.uniform(0.7, 1.0)
            mask_scaled = mask * alpha
            blended = (foreground_rgb * mask_scaled + 
                      background * (1 - mask_scaled)).astype(np.uint8)
            
        else:
            # Default to alpha blending
            blended = (foreground_rgb * mask + 
