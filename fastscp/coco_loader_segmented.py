...
class COCOLoaderSegmented:
    ...
    def __init__(self, annotation_file: str, image_dir: Optional[str] = None, 
                 cache_size: int = 1000, use_bbox_fallback: bool = True):
        """
        ...
        self._cache: Dict[int, Tuple[np.ndarray, np.ndarray]] = {}  # annotation_id -> (cropped object, mask)
    ...
    def _create_mask_from_segmentation(self, ann: Dict[str, Any], img_shape: Tuple[int, int]) -> np.ndarray:
    ...
    def load_object_crop(self, annotation_id: int, include_context: float = 0.0) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        """
    ...
    def get_random_objects(self, category: str, count: int, include_context: float = 0.0) -> List[Tuple[np.ndarray, np.ndarray]]:
        """
